...
export class OptionsService {
  ...
  private otherApis: OtherApi = {
    Proxies: {
      ...
      code: "code",
      desc: (i: IntegrationProfile) =>
        i.code.concat(i.description ? ` (${i.description})` : ""),
    },
    Licenses: {
      uri: "/acq/licenses",
      code: "code",
  ...
  get options(): Observable<Options> {
    return iif(
      ...
      forkJoin(
        /* Code tables */
        this.codes.map((key) =>
          this.restService.call<CodeTable>(`/conf/code-tables/${key}`)
        )
      ).pipe(
        tap((results) => {
        ...
        /* Other APIs */
        switchMap(() =>
          forkJoin(Object.values(this.otherApis).map((a) => this.getAll(a.uri)))
        ),
        tap((results) => {
          Object.entries(this.otherApis).forEach(([k, v], i) => {
            this._options[camelCase(k)] = findArray(results[i])
  ...
  getAll<T = any>(
    ...
    options: { arrayName: string; chunkSize: number } = {
      arrayName: null,
      chunkSize: 50,
    }
  ) {
      ...
      switchMap((results) =>
        iif(
          () =>
            !(
              arrayName &&
              Array.isArray(results[arrayName]) &&
              count > results[arrayName].length
            ),
          of(results as T),
          forkJoin(
            arrayOf(Math.ceil(count / chunkSize) - 1).map((i) => {
            ...
            map((results) => {
              ...
              return Object.assign(
                results[0],
                Object.assign(results[0], { [arrayName]: array })
              ) as T;
            })
          )
        )
...
const findArrayName = (obj: Object) =>
